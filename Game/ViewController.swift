...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var main_LBL_LeftPlayer: UILabel!
    @IBOutlet weak var main_LBL_RightPlayer: UILabel!
    var timer:myTimer?
    let iterations = 10
    var scoreLeft:Int = 0
...
extension ViewController: CallBack_Timer {
    ...
    func tickDetected(iter: Int) {
        ...
        if (iter == iterations){
            saveWinner()
            performSegue(withIdentifier: "scoreController", sender: nil)
...
extension ViewController: cards {
    ...
    private func setSides() {
        let side = UserDefaults.standard.string(forKey: "LocationSide")
        if side == "West"{
            main_LBL_LeftPlayer.text = UserDefaults.standard.string(forKey: "userName")
            main_LBL_RightPlayer.text = "bot"
        }
        else{
            main_LBL_RightPlayer.text = UserDefaults.standard.string(forKey: "userName")
            main_LBL_LeftPlayer.text = "bot"
        }
    }
...
extension ViewController: score {
    ...
    func getWinner() -> String {
        if (scoreLeft > scoreRight) {
            return (main_LBL_LeftPlayer.text ?? "0") + " won with " + String(scoreLeft) + " points"
        }
        return (main_LBL_RightPlayer.text ?? "0") + " won with " + String(scoreRight) + " points"
    }
...
extension ViewController: board {
    ...
    func saveWinner() {
        ...
        UserDefaults.standard.set(getWinner(), forKey: "WinnerString")
        
        
    }
    
    func resetBoard() {
        main_LBL_title.text = "War!!!"
